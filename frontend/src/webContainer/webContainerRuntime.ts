...
import type { FixState } from "../lib/types";
import { getWebContainer } from "./webContainerManager";
import { store } from "../lib/store";
import { isWebcontainerLoadedAtom } from "../lib/Atoms";
...
function filterBuildError(logContent: string) {
  ...
  for (const line of lines) {
    ...
    if (
      isErrorBlock &&
      stackTraceKeywords.some((keyword) => line.includes(keyword))
    ) {
